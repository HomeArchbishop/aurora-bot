import { createReplyDecorator } from '@/extensions/chat'
import { encryptText } from '@/services/hatimi'
...
export const replyDecorator = createReplyDecorator(async (splits, ctx) => {
  ...
  for (const split of splits) {
    ...
    if (ctx.domain.flags.isHajiami) {
      processedSplits.push(encryptText(split))
      continue
    }
